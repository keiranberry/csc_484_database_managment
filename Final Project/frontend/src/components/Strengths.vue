<template>
    ...
    <Card>
        <template #title>Get Strengths For Pokemon</template>
        ...
        <template #footer>
        <DataTable v-if="hasRetrieved" :value="strengthList">
...
const getStrengths = async () => {
    ...
    try {
        const response = await axios.get(
            `http://localhost:5000/pokemon/${selectedPokemon.value.Id}/strengths`
        );
        strengthList.value = [
        ...
        toast.add({
            severity: 'error',
            summary: 'Error',
            detail: 'Failed to fetch strengths.',
            life: 3000
        });
    }
}
